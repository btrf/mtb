...
async function exportJson() {
  ...
  a.download = 'mtb-export.json';
  a.click();
}

async function exportDb() {
  if (isAndroid) { Android.exportJson(); return; }
  const a = document.createElement('a');
  a.href = API + '/export/db';
  a.download = 'mtb-service.db';
  a.click();
}

function showImportModal() {
  if (isAndroid) {
    showModal(`<h2>${t('import_title')}</h2>
      <p style="color:var(--text-muted);font-size:13px;margin-bottom:12px">${t('import_warn')}</p>
      <div style="margin-bottom:10px">
        <input type="file" id="import-json-input" accept=".json" style="display:none" onchange="importFromJson()">
        <button class="btn btn-primary" onclick="document.getElementById('import-json-input').click()" style="width:100%;padding:16px;font-size:15px">
          <div style="font-size:28px;margin-bottom:6px">📥</div>
          <div style="font-weight:700">${t('import_title')}</div>
          <div style="font-size:12px;color:var(--text-muted);margin-top:4px">${t('from_json')}</div>
        </button>
      </div>
    ...
    showModal(`<h2>${t('import_title')}</h2>
      <p style="color:var(--text-muted);font-size:13px;margin-bottom:12px">${t('import_warn')}</p>
      <div style="margin-bottom:10px">
        <label style="font-size:13px;font-weight:600;color:var(--text-muted)">${t('from_json')}</label>
        <input type="file" id="import-json-input" accept=".json" style="display:block;margin-top:4px;color:var(--text)">
        <button class="btn btn-primary btn-sm" onclick="importFromJson()" style="margin-top:6px">${t('import_btn')}</button>
      </div>
      <div style="border-top:1px solid var(--border);padding-top:10px">
        <label style="font-size:13px;font-weight:600;color:var(--text-muted)">${t('from_db')}</label>
        <input type="file" id="import-db-input" accept=".db,.sqlite,.sqlite3" style="display:block;margin-top:4px;color:var(--text)">
        <button class="btn btn-primary btn-sm" onclick="importFromDb()" style="margin-top:6px">${t('import_db_btn')}</button>
      </div>
      <div class="form-actions">
        <button type="button" class="btn btn-outline" onclick="hideModal()">${t('close')}</button>
      </div>
...
function importFromJson() {
  ...
  reader.onload = function () {
    try { JSON.parse(reader.result); } catch { return alert(t('invalid_json')); }
    if (isAndroid) {
      Android.importJson(reader.result);
    } else {
      fetch(API + '/import/json', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: reader.result });
    }
    hideModal();
    renderAll();
  };
...
function importFromDb() {
  ...
  reader.onload = function () {
    ...
    fetch(API + '/import/db', { method: 'POST', body: buf });
    hideModal();
    renderAll();
  };
...
function showExportModal() {
  showModal(`<h2>${t('export_title')}</h2>
    <p style="color:var(--text-muted);font-size:13px;margin-bottom:14px">${t('export_desc')}</p>
    <div style="display:grid;grid-template-columns:1fr 1fr;gap:10px">
      ${isAndroid ? '' : `<button class="btn btn-outline" onclick="exportJson();hideModal()" style="padding:20px;flex-direction:column;height:auto;font-size:15px">
        <div style="font-size:28px;margin-bottom:6px">📄</div>
        <div style="font-weight:700">${t('export_json')}</div>
        <div style="font-size:12px;color:var(--text-muted);margin-top:4px">${t('export_json_desc')}</div>
      </button>`}
      <button class="btn btn-outline" onclick="exportDb();hideModal()" style="padding:20px;flex-direction:column;height:auto;font-size:15px">
        <div style="font-size:28px;margin-bottom:6px">🗄️</div>
        <div style="font-weight:700">${t('export_db')}</div>
        <div style="font-size:12px;color:var(--text-muted);margin-top:4px">${t('export_db_desc')}</div>
      </button>
    </div>
...
function showSettingsModal() {
  if (isAndroid) {
    showModal(`<h2>${t('settings_title')}</h2>
      ...
      <div style="display:grid;grid-template-columns:1fr 1fr;gap:10px">
        <input type="file" id="import-json-input" accept=".json" style="display:none" onchange="importFromJson()">
        <button class="btn btn-outline" onclick="hideModal();exportDb()" style="padding:20px;flex-direction:column;height:auto;font-size:15px">
          <div style="font-size:28px;margin-bottom:6px">📤</div>
    ...
    showModal(`<h2>${t('settings_title')}</h2>
      <p style="color:var(--text-muted);font-size:13px;margin-bottom:14px">${t('settings_desc')}</p>
      <div style="display:grid;grid-template-columns:1fr 1fr;gap:10px">
        <button class="btn btn-outline" onclick="showExportModal()" style="padding:20px;flex-direction:column;height:auto;font-size:15px">
          <div style="font-size:28px;margin-bottom:6px">📤</div>
          <div style="font-weight:700">${t('export_title')}</div>
        </button>
        <button class="btn btn-outline" onclick="showImportModal()" style="padding:20px;flex-direction:column;height:auto;font-size:15px">
          <div style="font-size:28px;margin-bottom:6px">📥</div>
          <div style="font-weight:700">${t('import_title')}</div>
        </button>
      </div>
